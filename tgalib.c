...
static void VerticalFlip(tImageTGA *image) 
{
	...
	rowlen  = image->sizeX * image->channels;
	numrows = image->sizeY & 1 ? image->sizeY - 1 / 2 : image->sizeY / 2;
	row     = malloc(rowlen);
	...
	for (i = 0; i < numrows; i++) 
	{
		memcpy(row, &image->data[i * rowlen], rowlen);
		memcpy(&image->data[i * rowlen], &image->data[(image->sizeY - i - 1) * rowlen], rowlen);
		memcpy(&image->data[(image->sizeY - i - 1) * rowlen], row, rowlen);
	}
...
tImageTGA *LoadTGA(const char *filename)
{
	...
	if(imageType != TGA_RLE)
	{
		...
		if(bits == 24 || bits == 32)
		{
			...
			pImageData->data = malloc(stride * height);
